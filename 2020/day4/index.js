let input = `pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
...
const rules = {
    ...
    'hgt': (x) => {
        if (x.match(/(cm)/)) {
            const num = Number(x.split('cm')[0]);

            if (num < 150 || num > 193) {
                return false;
            }
        } else if (x.match(/(in)/)) {
            const num = Number(x.split('in')[0]);
            if (num < 59 || num > 76) {
                return false;
            }
        } else {
    ...
    'hcl': (x) => {
        return !!x.match(/^#([0-9a-f]{6})$/);
    },
    'ecl': (x) => {
        return !!x.match(/^(amb|blu|brn|gry|grn|hzl|oth)?$/);
    },
    'pid': (x) => {
        return !!x.match(/^[0-9]{9}$/);
    }
}
